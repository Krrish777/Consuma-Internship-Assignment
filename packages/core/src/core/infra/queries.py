...
async def begin_parse(session: AsyncSession, job_id: str, n_blocks: int) -> bool:
    """Initialise ``pending_count`` exactly once, on the first CAS out of PENDING.
    ...
    untouched (this is the *normal* concurrent outcome, not an error); return
    False so the caller skips counter init and merely re-publishes the N
    TtsRequested events (parse is a fan-out emitter that must stay re-runnable).

    NOTE on the target state: the FSM (``core/domain/state.py``) makes PARSING the
    only legal successor of PENDING — a direct PENDING→GENERATING jump is illegal.
    The PARSING→GENERATING advance is a separate
...
async def advance_status(session: AsyncSession, job_id: str, to_status: JobStatus) -> bool:
    """Compare-and-set the job status to ``to_status`` (the contract).
...
async def finalize_job(session: AsyncSession, job_id: str, final_key: str) -> bool:
    """CAS STITCHING→COMPLETED and record ``final_key`` (the contract).
...
async def job_counts_by_status(session: AsyncSession) -> dict[str, int]:
    """Per-status job counts for ``GET /stats``.

"""R3.3 — poison-pill / DLQ probe (L4): dead-letter after 3 retries, no HOL block.
...
``q.parse``. W7's DLQ resolver then converges the poison job (parse-poison →
...
async def test_poison_dlqs_after_retries_without_head_of_line_block(
    ...
    # Poison exhausts 3 retries (~1+4+16s) → q.dlq → W7 resolves the parse-poison

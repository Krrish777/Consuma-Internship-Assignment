"""Stitch + webhook probe: final asset produced; webhook never fails the job.

Two properties:
  * stitch — the happy path concatenates the block chunks into ``out/<job>.mp3``
    and the job reaches COMPLETED;
  * MUST #8 — a webhook is best-effort: a job with a ``callback_url`` set still
    COMPLETEs with its asset regardless of the notification outcome.

Scope note (honest split): actual webhook *delivery* ("received once") and the
attempted-then-swallowed failure are proven (test_webhook, 3 tests, with the SSRF
resolver mocked). They are not re-exercised here because the SSRF guard correctly
